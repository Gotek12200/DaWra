...
```{r}
# recombining words into reviews for the DTM
train_docs <- clean_train %>%
  group_by(id) %>%
  summarize(review = paste(word, collapse = " "))

# create index-tokens
it_train <- itoken(train_docs$review, progressbar = FALSE)

# collecting unique terms
train_vocab <- create_vocabulary(it_train)

# filtering infrequent terms
train_vocab <- prune_vocabulary(train_vocab, term_count_min=5)
...
# Running the model
nmf_res <- nmf(dtm_dense, k = k, seed = 123)
