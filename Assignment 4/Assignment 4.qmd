...
# use the models we  trained
X_counts <- dtm_matrix
km5_clusters  <- kmeans_k5$km$cluster
km10_clusters <- kmeans_k10$km$cluster
km5_centers   <- kmeans_k5$km$centers
...
```
Among all models, LDA with 5 topics demonstrates the best overall clustering performance.
It achieves strong internal validity (Silhouette = 0.61, DB = 1.20), indicating well-defined and clearly separated clusters.
NMF also shows high compactness (Silhouette = 0.69) but a much higher DB index (10.25), suggesting that while documents within each cluster are similar, the clusters themselves are unevenly distributed or overlapping.
LDA with 10 topics performs slightly worse (Silhouette = 0.50, DB = 1.51), likely due to topic fragmentation as the number of clusters increases.
In contrast, K-Means and LSA + K-Means achieve very low silhouette values and high DB indices, reflecting poor separation and limited semantic structure in their clusters.
Overall, probabilistic models such as LDA and NMF are far more effective for uncovering meaningful, interpretable patterns in textual data than traditional distance-based clustering approaches.
