...
```{r}
# encoding nominal variables in one-hot encoding
...
```{r}
# converting binary variables to 0 or 1
...
train_split <- sample.split(train_wo_outliers$score, SplitRatio = 0.8)
val_split <- sample.split(train_wo_outliers$score[!train_split], SplitRatio = 0.5)
...
train_set <- train_wo_outliers[train_ids, ]
test_set <- train_wo_outliers[test_ids, ]
val_set <- train_wo_outliers[val_ids, ]
...
train_data_vif <- train_set[, !(names(train_set) %in% vars_to_remove)]
test_data_vif <- test_set[, !(names(test_set) %in% vars_to_remove)]
...
# defining model
model_deepl <- keras_model_sequential() %>%
...
# evaluation
model_deepl %>% evaluate(x_test, y_test, verbose = 1)
predictions_deepl <- model_deepl %>% predict(x_test)
```
...
```
Our best performing model is the deep learning, which slightly outperforms the KNN and linear regression. LR and KNN seem to be very similar in performance. Since there are no big differences between the models, we can assume that the data is mostly linear. 
...
ggplot(prediction_comparison, aes(x=Model, y=Scores, fill=Model)) +
  ...
  guides(fill="none")
```
If we look at the distribution of the data, we can see that none of the models capture the range of scores that the students actually had and all of them estimate scores to be closer to the mean. The deep learning has the highest range of predictions that is most similar to the test set, but it still doesn't cover all the scores. 

# Chosen model
We chose the deep learning as our final models, since it produces the lowest error and the most similar distribution of predictions to the test data.

### Preparing the test dataset
For the test dataset we also have to carry out the same steps.
Since the deep learning and KNN models are sensitive to scaling, we have to also normalize the test dataset to create accurate predictions. 
```{r}
...
# scaling 1-5 scores (5 will be 1 and 1 will be scaled to 0)
test_norm <- test_clean %>% mutate(across(c("famrel","freetime","goout","Dalc","Walc", "health"), ~ (. - 1) / 4))

# scaling 1-4 scores (4 will be 1 and 1 will be scaled to 0)
test_norm <- test_clean %>% mutate(across(c(Medu, Fedu), ~ . / 4))
...
# scaling using normal distribution
test_norm <- test_norm %>% mutate(across(c(failures, age, absences), ~ as.numeric(scale(.))))

nominal_vars <- c("Mjob","Fjob","reason","guardian")

test_one_hot <- dummy_cols(
  test_norm,
  select_columns = nominal_vars,
  remove_selected_columns = TRUE
)
...
# assigning TRUE or FALSE to binary values (the second possible value is assigned 1 the first 0)
test_one_hot <- test_one_hot %>% mutate(across(all_of(binary_vars), ~ as.integer(.x == levels(.x)[2])))
```

### Creating rds file with test predictions
```{r}
# creating predictions
x_test_data <- test_one_hot %>% as.matrix()
test_data_predictions <- model_deepl %>% predict(x_test)

# create vector & write rds
pred_vec <- as.vector(test_data_predictions)
