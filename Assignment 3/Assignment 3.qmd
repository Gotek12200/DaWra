...
# datasets for linear model (not normalized) 
train_linear <- train_clean[train_ids, ]
test_linear <- train_clean[test_ids, ]
val_linear <- train_clean[val_ids, ]

# dataset for k-nearest neighbors (normalized)
train_knn <- train_norm[train_ids, ]
test_knn <- train_norm[test_ids, ]
val_linear <- train_norm[val_ids, ]
...
```{r}
#Creating the model
lin_regression <- lm(score ~ ., data = train_linear)
...
predictions <- predict(lin_regression, newdata = test_linear)
true_score <- test_linear$score
...
pred <- predict(model_1, newdata = test_knn)
