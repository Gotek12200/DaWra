...
```
# splitting dataset
here we split the data into our test, train and validation set

```{r}
set.seed(123)
#first split it 80 20 for the train and test/val set
split1 <- sample.split(train_cleaned$score, SplitRatio = 80)
train_data <- subset(train_cleaned, split1 == TRUE)
test_val_set <- subset(train_cleaned, split1 == FALSE)

#split the val set 50/50 so each has 10% of the total set
split2 <- sample.split(test_val_set$score, SplitRatio = 50)
test_data <- subset(test_val_set, split2 == TRUE)
val_data <- subset(test_val_set, split2 == TRUE)
```
# Model description
