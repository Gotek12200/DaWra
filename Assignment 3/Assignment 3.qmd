...
predictions_knn <- predict(model_1, newdata = test_set)
...
differences <- predictions_knn - pred_wo_outliers
summary(differences)
```

## Deep learning model
For deep learning we use 2 hidden layers with 32 and 16 nodes, with ReLu activations. There is also a dropout of 0.2 between layers. Since the training data has relatively few entries, we only do a few epochs to avoid overfitting. For the training the ADAM optimizer is used and the loss function is the mean-squared error.
```{r}
...
To compare the models we are using Mean Average Error and Mean Squared Error. 
...
- Fani Profiti: outlier detection & performance comparison, k-nearest neighbors model
- Author Three: a, b, d
